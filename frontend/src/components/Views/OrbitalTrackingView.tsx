    ...
    <div className="flex flex-col flex-1 p-5 bg-bg font-mono select-none overflow-y-auto">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-line mb-4">
        ...
        <div className="flex items-center gap-2.5 text-xs">
          <div className="hud-glass px-3 py-1.5 rounded border border-line flex items-center gap-2">
            <span className="w-1.5 h-1.5 rounded-full bg-cyan dot-pulse" />
            <span className="text-muted">CARRIER:</span>
            <span className="text-cyan font-bold">LOCKED (S/X-BAND)</span>
          </div>
          <div className="hud-glass px-3 py-1.5 rounded border border-line flex items-center gap-2">
            <span className="text-muted">SNR:</span>
            <span className="text-safe font-bold">19.4 dB</span>
          </div>
          <div className="hud-glass px-3 py-1.5 rounded border border-line flex items-center gap-2">
            <span className="text-muted">BER:</span>
            <span className="text-cyan font-bold">3.8 &times; 10&minus;8</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
        ...
        <div className="lg:col-span-2 bg-[#041A0B] p-4 rounded border border-line reticle-corner shadow-panel-subtle flex flex-col">
          <div className="flex items-center justify-between mb-3">
            <div className="text-xs font-display font-bold text-cyan tracking-wider uppercase flex items-center gap-2">
              <span className="text-cyan">&gt;&gt;</span> Global Ground Station Footprint &amp; Sub-Satellite Track
            </div>
          ...
          <div className="relative h-[290px] bg-[#021408] rounded border border-line/60 overflow-hidden flex items-center justify-center">
            ...
            <svg viewBox="0 0 720 360" className="w-full h-full block">
              ...
              <line x1="0" y1="180" x2="720" y2="180" stroke="#1D4ED8" strokeWidth="1.2" />
              <line x1="360" y1="0" x2="360" y2="360" stroke="#1D4ED8" strokeWidth="1.2" />

              {/* Stylized Continents Outlines in Deep Space Blue */}
              {/* Asia & India */}
              <path
              ...
              {/* Ground Stations on Map in Blue / Cyan */}
                ...
                return (
                  <g key={gs.code}>
                    ...
                    <circle
                      ...
                      cy={cy}
                      r="45"
                      fill={isActive ? '#00F0FF' : '#334155'}
                      fillOpacity={isActive ? '0.14' : '0.04'}
                      stroke={isActive ? '#00F0FF' : '#1E3A8A'}
                      strokeWidth="1"
              ...
              {/* Current Satellite Sub-point position in Green */}
              {(() => {
                const satX = 360 + (currentLon / 180) * 360
                const satY = 180 - (currentLat / 90) * 180
                return (
                  <g>
                    {/* Orbital path projection in Cyan */}
                    <path
                    ...
                    {/* Target reticle beacon in Green */}
                    <circle cx={satX} cy={satY} r="7" fill="none" stroke="#00FF87" strokeWidth="1.5">
                      <animate attributeName="r" values="5;10;5" dur="1.5s" repeatCount="indefinite" />
                    </circle>
                    <circle cx={satX} cy={satY} r="3" fill="#00FF87" />
        ...
        <div className="bg-[#041A0B] p-4 rounded border border-line reticle-corner shadow-panel-subtle flex flex-col justify-between">
          <div>
            <div className="text-xs font-display font-bold text-cyan tracking-wider uppercase mb-3 flex items-center gap-2">
              <span className="text-cyan">&gt;&gt;</span> Orbital Mechanics Telemetry
            </div>

            <div className="space-y-2 text-xs">
              <div className="flex justify-between py-1 border-b border-dashed border-line/60">
                <span className="text-muted">Orbit Class:</span>
                <span className="text-slate-100 font-bold">Sun-Synchronous LEO (SSO)</span>
              </div>
              <div className="flex justify-between py-1 border-b border-dashed border-line/60">
                <span className="text-muted">Mean Altitude:</span>
                <span className="text-cyan font-bold">520.42 km</span>
              </div>
              <div className="flex justify-between py-1 border-b border-dashed border-line/60">
                <span className="text-muted">Orbital Velocity:</span>
                <span className="text-slate-100 font-bold">7.612 km/s (27,403 km/h)</span>
              </div>
              <div className="flex justify-between py-1 border-b border-dashed border-line/60">
                <span className="text-muted">Inclination:</span>
              ...
              <div className="flex justify-between py-1 border-b border-dashed border-line/60">
                <span className="text-muted">Apogee / Perigee:</span>
                <span className="text-slate-100 font-bold">524.1 km / 516.8 km</span>
              </div>
              <div className="flex justify-between py-1 border-b border-dashed border-line/60">
                <span className="text-muted">Doppler Shift:</span>
                <span className="text-safe font-bold">+18.42 kHz</span>
              </div>
              <div className="flex justify-between py-1 border-b border-dashed border-line/60">
                <span className="text-muted">Downlink Packets:</span>
                <span className="text-cyan font-bold">PKT #{telemetryTick}</span>
              </div>
              <div className="flex justify-between py-1">
                <span className="text-muted">Frame Sync Status:</span>
                <span className="text-safe font-bold">LOCKED &bull; 0 CRC ERR</span>
              </div>
            </div>
          </div>

          <div className="mt-4 p-2.5 rounded bg-[#062612] border border-cyan/40 text-[10px]">
            <div className="text-cyan font-bold mb-1 uppercase">Next Ground Station Pass:</div>
            <div className="text-slate-200">
              <b>Bengaluru ISTRAC NOCC</b> &bull; AOS in <span className="text-safe font-bold">04m 18s</span> &bull; Max Elevation: 74&deg;
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="rounded border border-line bg-[#041A0B] p-3 reticle-corner">
        <div className="text-xs font-display font-bold text-cyan tracking-wider uppercase mb-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-2 text-xs">
          {GROUND_STATIONS.map((gs) => (
            <div
              key={gs.code}
              className={`p-2 rounded border font-mono text-[10.5px] ${
                gs.status === 'ACTIVE'
                  ? 'bg-cyan/15 border-cyan text-white shadow-neon-cyan'
                  : gs.status === 'ACQUIRING'
                  ? 'bg-safe/15 border-safe text-white'
                  : 'bg-[#031709] border-line text-muted'
              }`}
            >
              ...
              <div className="truncate text-[9.5px] mt-0.5">{gs.name}</div>

...
export function generateExcelReport(
  ...
  rows.push([
    ...
    'Subsystem Name',
    'Qualification Lot',
    'V0 Baseline Current (uA)',
    'V24 Early Burn-In (uA)',
    'V96 Mid Burn-In (uA)',
    'V168 Final Qualified (uA)',
    'Datasheet Absolute Limit (uA)',
    'Parametric Drift Rate (%)',
  ...
  for (const part of data) {
    ...
    rows.push([
      ...
      `${(part.pct_drift ?? 0) > 0 ? '+' : ''}${(part.pct_drift ?? 0).toFixed(2)}%`,
